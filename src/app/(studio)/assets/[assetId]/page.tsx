  ...
  return (
    <main className="asset-detail-page">
      ...
      <div className="asset-detail-grid">
        <section className="asset-preview-section" aria-label="Model preview" data-version-id={asset.currentVersionId}>
          {asset.status === "ready" && url && primary ? <ModelCanvasLoader key={asset.currentVersionId} modelUrl={url} primaryRelativePath={primary.relativePath} files={asset.files.map((file) => ({ relativePath: file.relativePath, storageKey: file.storageKey }))} /> : <div className="asset-preview-unavailable"><Box size={34} aria-hidden="true" /><h2>Preview not ready</h2><p>{asset.status === "failed" ? "This import did not complete. Return to the library and try the source package again." : "The model is still being prepared."}</p></div>}
        </section>
