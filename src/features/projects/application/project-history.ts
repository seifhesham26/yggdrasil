...
export class InMemoryProjectRepository implements ProjectRepository {
  ...
  async undo(ownerId: string, projectId: string): Promise<ProjectState> { const project = this.owned(ownerId, projectId); if (!project.currentRevisionId) throw new Error("No project revision is available to undo."); const current = this.revisions.get(project.id)!.find((revision) => revision.id === project.currentRevisionId)!; if (!current.parentRevisionId) throw new Error("No earlier project revision is available."); const parent = this.revisions.get(project.id)!.find((revision) => revision.id === current.parentRevisionId)!; Object.assign(project, { currentRevisionId: parent.id, snapshot: parent.snapshot, updatedAt: new Date() }); return this.state(project); }
