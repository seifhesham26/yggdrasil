  ...
  return (
    <Link className="asset-row" href={`/assets/${asset.id}`} aria-label={`Open ${asset.name}`}>
      ...
      <span className="asset-row-stats" aria-label="Model statistics">
        <span><Layers3 size={16} aria-hidden="true" /> {countLabel(asset.counts?.meshes, "mesh", "meshes")}</span>
        <span><ScanLine size={16} aria-hidden="true" /> {countLabel(asset.counts?.triangles, "triangle", "triangles")}</span>
        <span><Film size={16} aria-hidden="true" /> {countLabel(asset.counts?.animations, "animation", "animations")}</span>
      </span>
